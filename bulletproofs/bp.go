...
func Prove(secret *big.Int, params BulletProofSetupParams) (BulletProof, *big.Int, error) {
	...
	/*
		tmp := new(big.Int).Add(secret, big.NewInt(101))
		tmp2 := new(p256.P256).ScalarBaseMult(tmp)
		tmp3 := new(p256.P256).ScalarMult(params.H, gamma)
		fmt.Println("Lo:", tmp)
		fmt.Println("Lo:", new(p256.P256).Add(tmp2, tmp3))

		tmp = new(big.Int).Sub(secret, big.NewInt(1000))
		tmp = new(big.Int).Sub(big.NewInt(0), tmp)
		tmp2 = new(p256.P256).ScalarBaseMult(tmp)
		tmp3 = new(p256.P256).ScalarMult(params.H, new(big.Int).Sub(ORDER, gamma))
		fmt.Println("Hi:", tmp)
		fmt.Println("Hi:", new(p256.P256).Add(tmp2, tmp3))
	*/
