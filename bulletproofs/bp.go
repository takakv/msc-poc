...
/*
BulletProofSetupParamsSP is the structure that stores the parameters for
the Zero Knowledge Proof system.
*/
type BulletProofSetupParamsSP struct {
	// N is the bit-length of the range.
	N int64
	// G is the Elliptic Curve generator.
	G algebra.Element
...
/*
BulletProofSP structure contains the elements that are necessary for the verification
of the Zero Knowledge Proof.
*/
type BulletProofSP struct {
	V                 algebra.Element
	VEl               algebra.Element
	A                 algebra.Element
	S                 algebra.Element
	...
	Params            BulletProofSetupParamsSP
}
...
func SetupSP(b int64, SP algebra.Group) (BulletProofSetupParamsSP, error) {
	if !IsPowerOfTwo(b) {
		return BulletProofSetupParamsSP{}, errors.New("range end is not a power of 2")
	}
	...
	params.N = int64(math.Log2(float64(b)))
	if !IsPowerOfTwo(params.N) {
		return BulletProofSetupParamsSP{}, fmt.Errorf("range end is a power of 2, but it's exponent should also be. Exponent: %d", params.N)
	}
	if params.N > 32 {
		return BulletProofSetupParamsSP{}, errors.New("range end can not be greater than 2**32")
	}
...
func ProveSP(secret *big.Int, params BulletProofSetupParamsSP) (BulletProofSP, *big.Int, error) {
	var (
		proof BulletProofSP
	)
	// ////////////////////////////////////////////////////////////////////////////
	...
	aR, _ := computeAR(aL)                                                                  // (42)
	alpha, _ := rand.Int(rand.Reader, ORDER)                                                // (43)
	A := commitVectorSP(aL, aR, alpha, params.H, params.Gg, params.Hh, params.N, params.SP) // (44)

	// sL, sR and commitment: (S, rho)                                     // (45)
	sL := sampleRandomVector(params.N)
	sR := sampleRandomVector(params.N)
	rho, _ := rand.Int(rand.Reader, ORDER)                                                   // (46)
	S := commitVectorBigSP(sL, sR, rho, params.H, params.Gg, params.Hh, params.N, params.SP) // (47)
	...
	*/
	...
	// Inner Product over (g, h', P.h^-mu, tprime)
	hprime := updateGeneratorsSP(params.Hh, y, params.N, params.SP)
...
func (proof *BulletProofSP) VerifySP() (bool, error) {
	params := proof.Params
	// Recover x, y, z using Fiat-Shamir heuristic
...
func updateGeneratorsSP(Hh []algebra.Element, y *big.Int, N int64, SP algebra.Group) []algebra.Element {
	var (
		i int64
	)
	// Compute h'                                                          // (64)
...
func commitVectorBigSP(aL, aR []*big.Int, alpha *big.Int, H algebra.Element, g, h []algebra.Element, n int64, SP algebra.Group) algebra.Element {
	// Compute h^alpha.vg^aL.vh^aR
	// R := new(p256.P256).ScalarMult(H, alpha)
...
func commitVectorSP(aL, aR []int64, alpha *big.Int, H algebra.Element, g, h []algebra.Element, n int64, SP algebra.Group) algebra.Element {
	// Compute h^alpha.vg^aL.vh^aR
	// R := new(p256.P256).ScalarMult(H, alpha)
	R := SP.Element().Scale(H, alpha)
	for i := int64(0); i < n; i++ {
...
func (params *BulletProofSetupParamsSP) delta(y, z *big.Int) *big.Int {
	var (
		result *big.Int
	)
	// delta(y,z) = (z-z^2) . < 1^n, y^n > - z^3 . < 1^n, 2^n >
